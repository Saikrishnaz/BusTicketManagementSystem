...
Public Class BookTicket
    ...
    Private Function IsAllPassengerFieldsFilled() As Boolean
        ...
        Return Not (
            ...
            String.IsNullOrWhiteSpace(Txt_BusType.Text) OrElse
    ...
    Private Sub ClearAllPassengerBusControls()
        ...
        Dim controlsToClear() As Control = {Txt_ArrivalTime, Txt_BusType, Txt_BusType, Txt_Departure, Txt_Destination, Txt_FairPrice, Txt_PsgnMobileNo, Txt_PsgnName, Txt_SourceLocation, Txt_TickenNo}
    ...
    Private Sub Guna2GradientButton3_Click(sender As Object, e As EventArgs) Handles Guna2GradientButton3.Click
        ...
        ' Check if the passenger name already exists in the "BookedTicketData" table
        If CHECKeMAIL(con, Txt_PsgnName.Text, "BookedTicketData", "PsgnName") Then
            ...
            Else
                ' Check if the same passenger has already booked a ticket for the same journey name
                If CHECKeMAIL(con, Txt_JourneyName.SelectedValue.ToString(), "BookedTicketData", "JourneyName") Then
                    MsgBox("Same Passenger Cannot Book Two Tickets for the Same Date...")
                    Exit Sub
                End If
            End If
        End If
        ...
        Try
            con.Open()
            Dim query As String = "INSERT INTO BookedTicketData (PsgnName, PsgnGender, PsgnMobileNo,TicketNumber,JourneyDate,JourneyName,BusName,BusType,SourceLocation,Depature,Destination,ArrivalTime,TicketFairPrice) " &
                        "VALUES (@PsgnName, @PsgnGender, @PsgnMobileNo,@TicketNumber,@JourneyDate,@JourneyName,@BusName,@BusType,@SourceLocation,@Depature,@Destination,@ArrivalTime,@TicketFairPrice)"
            Using cmd As New SqlCommand(query, con)
                ...
                cmd.Parameters.AddWithValue("@JourneyName", If(Txt_JourneyName.SelectedIndex <> -1, Txt_JourneyName.SelectedValue.ToString(), DBNull.Value))
                cmd.Parameters.AddWithValue("@BusName", If(Not String.IsNullOrEmpty(Txt_BusName.Text), Txt_BusName.Text, DBNull.Value))
                cmd.Parameters.AddWithValue("@BusType", If(Not String.IsNullOrEmpty(Txt_BusType.Text), Txt_BusType.Text, DBNull.Value))
    ...
    Private Sub Txt_JourneyName_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles Txt_JourneyName.SelectionChangeCommitted
        ...
        If reader.Read() Then
            ...
            Txt_BusName.Text = reader("BusName").ToString()
            Txt_BusType.Text = reader("BusType").ToString()
        End If
        con.Close()
    ...
    Private Sub PD_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PD.PrintPage
        ...
        e.Graphics.DrawString("Booked By Bus Ticket Master", f8b, Brushes.Black, 75, 7, Center)
        e.Graphics.DrawString(Txt_JourneyName.Text, f8, Brushes.Black, 75, 13, Center)
        e.Graphics.DrawString(Txt_BusName.Text, f7, Brushes.Black, 75, 20, Center)
